...
class DataValidation:
    def __init__(self,
    data_validation_config: DataValidationConfig,data_ingestion_artifact: DataIngestionArtifact):
        try:
    ...
    def validate_number_of_columns(self,dataframe: pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f'Number of required columns are {number_of_columns}')
    ...
    def detect_dataset_drift(self,base_dataset,current_dataset,threshold=0.05) -> dict:
        status = True
        report = {}

        for column in base_dataset.columns:
            d1 = base_dataset[column]
            d2 = base_dataset[column]
        ...
        write_yaml(content=report,file_path=drift_report_file_path)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not status:
                error_message='Train dataframe does not contain the same number of columns'

            status = self.validate_number_of_columns(dataframe=test_df)
            if not status:
                error_message='Test dataframe does not contain the same number of columns'
            
            status = self.detect_dataset_drift(base_dataset = train_df, current_dataset = test_df)
